...
<?php
//mysqli query
$sql = "SELECT order_id, isbn, title, author, publisher, course, CAST((cost * total) AS decimal(38,2)) 'totalprice', semester, start_date, campus, ins_name, ins_phone, ins_email, dir_name, dir_phone, dir_email, head_name, head_phone, head_email, total, status, comments FROM orders ORDER BY order_id DESC";
...
//loop that displays all of the data from each row while the number of rows is greater than 0
        ...
        while ($row = $result->fetch_assoc()){
          ...
          <tr>
            ...
            <td>
                <a href="orders_edit.php?edit=<?php echo $row['order_id']; ?>" alt="edit"
                 class="btn">Edit</a>
            </td>
            <td>
                 <a href="delete.php?delete_order=<?php echo $row['order_id']; ?>"
                   onclick="return confirm('Are you sure you want to delete this order?');" class="btn">Delete</a>
            </td>
               </tr>
               <?php
             }
